...
export class DataSonification extends Sonification{
    ...
    updateParameterMap(init = false){
        ...
        const dayData       = this.data.model[this.state.selection.sceneIndex],
        ...
        group.A.pitch.array = dayData.scaledData[group.A.pitch.interval].A.pitch.map(d => { return d[group.A.pitch.series].quantized})
        ...
        // i. Pitch: constructed from selected data => update params
        group.B.velocity.array = dayData.scaledData[group.B.velocity.interval].B.velocity.map(d => { return d[group.B.velocity.series].value})
        ...
        group.C["3"].array    = dayData.scaledData[group.C["3"].interval].C["3"].chord
                                    .map(d => d[group.C["3"].series].quantized)
