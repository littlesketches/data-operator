...
const Un = (t) => {
...
}, te = (t) => t.name || t.longname, Dn = (t) => t?.length ? `
  ...
  ({ name: n, type: a, description: o }) => `
          <li class="autocomplete-info-param-item">
            ...
            ${o ? `<div class="autocomplete-info-param-desc">${An(o)}</div>` : ""}
          </li>
        `
).join("")}
      </ul>
  ...
  ` : "", Cn = (t) => t?.length ? `
    <div class="autocomplete-info-examples-section">
      ...
      ${t.map(
  (n) => `
        <pre class="autocomplete-info-example-code">${Un(n)}</pre>
      `
).join("")}
    </div>
  ` : "", xe = ({ doc: t, label: n }) => Pn`
  <div class="autocomplete-info-container">
    <div class="autocomplete-info-tooltip">
      <h3 class="autocomplete-info-function-name">${n || te(t)}</h3>
...
`[0], Fn = (t) => {
...
}, Bn = (t) => ["superdirtOnly", "noAutocomplete"].some((n) => t.tags?.find((a) => a.originalTitle === n)), Sn = V.docs.filter((t) => Fn(t) && !Bn(t)).map((t) => ({
  label: te(t),
  ...
  return n.from === n.to && !t.explicit ? null : {
    from: n.from,
    options: Sn
    /*     options: [
...
}, qn = (t) => t ? [He({ override: [Nn], closeOnBlur: !1 })] : [], Tn = (t) => t.name || t.longname;
let G = !1;
...
const Mn = en(
  (t, n, a) => {
    if (!G)
      return null;
    let { from: o, to: s, text: i } = t.state.doc.lineAt(n), p = n, c = n;
    for (; p > o && /\w/.test(i[p - o - 1]); )
      p--;
    for (; c < s && /\w/.test(i[c - o]); )
      c++;
    if (p == n && a < 0 || c == n && a > 0)
      return null;
    let w = i.slice(p - o, c - o), g = V.docs.filter((U) => Tn(U) === w)[0];
    return !g && (g = V.docs.filter((U) => U.synonyms && U.synonyms.includes(w))[0], !g) ? null : {
      pos: p,
      end: c,
      above: !1,
      arrow: !0,
      create(U) {
        ...
        S.className = "strudel-tooltip";
        const A = xe({ doc: g, label: w });
        return S.appendChild(A), { dom: S };
      }
    };
  },
  { hoverTime: 10 }
), In = (t) => t ? Mn : [], $ = M.define(), En = R.define({
  create() {
    return v.none;
  },
  update(t, n) {
      ...
      for (let a of n.effects)
        if (a.is($))
          if (a.value && n.newDoc.length > 0) {
            ...
            t = v.set([o.range(0, n.newDoc.length)]);
          } else
            t = v.set([]);
      return t;
    } catch (a) {
  ...
  provide: (t) => P.decorations.from(t)
}), _n = (t, n = 200) => {
  t.dispatch({ effects: $.of(!0) }), setTimeout(() => {
    t.dispatch({ effects: $.of(!1) });
  }, n);
}, Rn = (t) => t ? En : [], ke = M.define(), ve = M.define(), Y = (t, n) => {
  t.dispatch({ effects: ke.of(n) });
}, Ln = (t, n, a) => {
  t.dispatch({ effects: ve.of({ atTime: n, haps: a }) });
}, K = R.define({
  create() {
    return v.none;
  },
  update(t, n) {
    ...
    for (let a of n.effects)
      if (a.is(ke)) {
        const o = a.value.filter(([s]) => s < n.newDoc.length).map(([s, i]) => [s, Math.min(i, n.newDoc.length)]).map(
          (s) => v.mark({
            id: s.join(":"),
      ...
      if (a.is(ve)) {
        ...
        for (let s of a.value.haps)
          if (!(!s.context?.locations || !s.whole))
            for (let { start: i, end: p } of s.context.locations) {
              let c = `${i}:${p}`;
              (!o.has(c) || o.get(c).whole.begin.lt(s.whole.begin)) && o.set(c, s);
            }
        t = { atTime: a.value.atTime, haps: o };
      }
    return t;
  }
}), On = P.decorations.compute([K, Z], (t) => {
  const n = t.field(K).iter(), { haps: a } = t.field(Z), o = new Je();
  for (; n.value; ) {
    const {
      from: s,
      to: i,
    ...
    if (a.has(p)) {
      const c = a.get(p), w = c.value?.color ?? "var(--foreground)", g = c.value?.markcss || `outline: solid 2px ${w}`;
      o.add(
        s,
        i,
        v.mark({
  ...
  return o.finish();
}), zn = [K, Z, On], Wn = (t, n) => (t && n && setTimeout(() => {
  Y(n.editor, n.miniLocations);
}, 100), t ? L.highest(zn) : []), Hn = ge.fromClass(
  class {
    constructor() {
    }
  },
  {
    provide: () => L.highest(C.of([...bn]))
  }
), Vn = (t) => [Hn].concat(t ?? []), Gn = {
  vim: fn,
  emacs: gn,
...
const r = ({ theme: t, settings: n, styles: a }) => {
  const o = P.theme(
    {
      "&": {
        color: n.foreground,
...
let k = "#6edee4", Ae = "white", De = "red", Ce = "#f8fc55";
const Fe = {
  background: "#000000",
  foreground: k,
  // whats that?
...
const Jn = r({
  ...
  settings: Fe,
  styles: [
    { tag: e.labelName, color: Ae },
    { tag: e.keyword, color: k },
    { tag: e.operator, color: I },
    { tag: e.special(e.variableName), color: k },
    { tag: e.typeName, color: k },
    { tag: e.atom, color: k },
    { tag: e.number, color: I },
    { tag: e.definition(e.variableName), color: k },
    { tag: e.string, color: I },
    { tag: e.special(e.string), color: I },
    { tag: e.comment, color: z },
    { tag: e.variableName, color: k },
    { tag: e.tagName, color: k },
    { tag: e.bracket, color: z },
    { tag: e.meta, color: k },
    { tag: e.attributeName, color: k },
    { tag: e.propertyName, color: k },
    // methods
    { tag: e.className, color: k },
    { tag: e.invalid, color: De },
    { tag: [e.unit, e.punctuation], color: z }
  ]
}), Qn = {
  ...
  selectionMatch: d[1],
  lineHighlight: d[3],
  lineBackground: d[3] + "90",
  //lineBackground: 'transparent',
  gutterBackground: "transparent",
  gutterForeground: d[0],
  light: !1
  // customStyle: '.cm-line { line-height: 1 }',
}, et = r({
  theme: "dark",
  settings: Be,
  styles: [
    { tag: e.comment, color: d[2] },
    { tag: e.string, color: d[1] },
    { tag: [e.atom, e.number], color: d[1] },
    { tag: [e.meta, e.labelName, e.variableName], color: d[0] },
    {
      tag: [e.keyword, e.tagName, e.arithmeticOperator],
      color: d[1]
    },
    { tag: [e.atom, e.bool, e.special(e.variableName)], color: d[0] },
    { tag: [e.function(e.variableName), e.propertyName], color: d[0] },
    { tag: e.atom, color: d[1] }
  ]
}), X = "#5c019a", nt = "#fbeffc", Se = "#272C35", tt = "#fee1ff", at = "#465063", ot = "#9acd3f", st = "#97a1b7", rt = "#f6a6fd", J = {
  background: "white",
  lineBackground: "transparent",
  foreground: X,
  caret: "#797977",
  ...
  styles: [
    ...
    { tag: [e.tagName, e.heading], color: J.foreground },
    { tag: e.comment, color: st },
    { tag: [e.variableName, e.propertyName, e.labelName], color: rt },
    { tag: [e.attributeName, e.number], color: "#d19a66" },
    { tag: e.className, color: Se },
    { tag: e.keyword, color: X },
    { tag: [e.string, e.regexp, e.special(e.propertyName)], color: ot }
  ]
}), j = ["#1e1e1e", "#fbde2d", "#ff1493", "#4c83ff", "#ededed", "#cccccc", "#ffffff30", "#dc2f8c"], Ne = {
  background: "#000000",
  lineBackground: "transparent",
  foreground: j[4],
  selection: j[6],
  gutterBackground: j[0],
  gutterForeground: j[5],
  gutterBorder: "transparent",
  lineHighlight: j[0]
}, lt = r({
  theme: "dark",
  ...
  styles: [
    ...
    { tag: [e.atom, e.bool, e.special(e.variableName)], color: j[3] },
    { tag: e.comment, color: "#54636D" },
    { tag: [e.variableName, e.propertyName, e.labelName], color: j[4] },
    { tag: [e.attributeName, e.number], color: j[3] },
    { tag: e.keyword, color: j[1] },
    { tag: [e.string, e.regexp, e.special(e.propertyName)], color: j[2] }
  ]
}), f = ["#000000", "#ff5356", "#bd312a", "#54636D", "#171717"], qe = {
  background: f[0],
  lineBackground: "transparent",
  foreground: f[2],
  selection: f[4],
  selectionMatch: f[0],
  gutterBackground: f[0],
  gutterForeground: f[3],
  gutterBorder: "transparent",
  lineHighlight: f[0]
}, mt = r({
  theme: "dark",
  ...
  styles: [
    ...
    { tag: [e.atom, e.bool, e.special(e.variableName)], color: f[1] },
    { tag: e.comment, color: f[3] },
    { tag: [e.variableName, e.propertyName, e.labelName], color: f[2] },
    { tag: [e.attributeName, e.number], color: f[1] },
    { tag: e.keyword, color: f[2] },
    { tag: [e.string, e.regexp, e.special(e.propertyName)], color: f[1] }
  ]
}), b = ["#000000", "#8ed675", "#56bd2a", "#54636D", "#171717"], Te = {
  background: b[0],
  lineBackground: "transparent",
  foreground: b[2],
  selection: b[4],
  selectionMatch: b[0],
  gutterBackground: b[0],
  gutterForeground: b[3],
  gutterBorder: "transparent",
  lineHighlight: b[0]
}, ct = r({
  theme: "dark",
  ...
  styles: [
    ...
    { tag: [e.atom, e.bool, e.special(e.variableName)], color: b[1] },
    { tag: e.comment, color: b[3] },
    { tag: [e.variableName, e.propertyName, e.labelName], color: b[2] },
    { tag: [e.attributeName, e.number], color: b[1] },
    { tag: e.keyword, color: b[2] },
    { tag: [e.string, e.regexp, e.special(e.propertyName)], color: b[1] }
  ]
}), y = ["rgb(0, 0, 0)", "rgb(82, 208, 250)", "rgba(113, 208, 250, .4)", "rgba(113, 208, 250, .15)"], Me = {
  background: y[0],
  lineBackground: "transparent",
  foreground: y[1],
  selection: y[2],
  selectionMatch: y[0],
  gutterBackground: y[0],
  gutterForeground: y[2],
  gutterBorder: "transparent",
  lineHighlight: y[0]
}, pt = r({
  theme: "dark",
  ...
  styles: [
    ...
    { tag: [e.atom, e.bool, e.special(e.variableName)], color: y[1] },
    { tag: [e.comment, e.brace, e.bracket], color: y[2] },
    { tag: [e.variableName, e.propertyName, e.labelName], color: y[1] },
    { tag: [e.attributeName, e.number], color: y[1] },
    { tag: e.keyword, color: y[1] },
    { tag: [e.string, e.regexp, e.special(e.propertyName)], color: y[1] }
  ]
}), h = [
  "rgb(84, 93, 98)",
  "rgb(255, 255, 255)",
  ...
  selection: h[8],
  selectionMatch: h[0],
  gutterBackground: h[3],
  gutterForeground: h[2],
  gutterBorder: "transparent",
  lineHighlight: h[3]
}, dt = r({
  theme: "dark",
  ...
  styles: [
    ...
    { tag: [e.bool, e.special(e.variableName)], color: h[1] },
    { tag: [e.comment, e.brace, e.bracket], color: h[2] },
    { tag: [e.variableName], color: h[1] },
    { tag: [e.labelName, e.propertyName, e.self, e.atom], color: h[5] },
    { tag: [e.attributeName, e.number], color: h[6] },
    { tag: e.keyword, color: h[5] },
    { tag: [e.string, e.regexp, e.special(e.propertyName)], color: h[4] }
  ]
}), x = ["rgb(75, 130, 247)", "rgb(47, 108, 246)", "rgb(255, 255, 255)", "rgba(255, 255, 255,.3)"], Ee = {
  background: x[0],
  lineBackground: "transparent",
  foreground: x[2],
  selection: x[3],
  ...
  styles: [
    ...
    { tag: [e.atom, e.bool, e.special(e.variableName)], color: x[2] },
    { tag: [e.comment, e.bracket, e.brace, e.compareOperator], color: x[3] },
    { tag: [e.variableName, e.propertyName, e.labelName], color: x[2] },
    { tag: [e.attributeName, e.number], color: x[2] },
    { tag: e.keyword, color: x[2] },
    { tag: [e.string, e.regexp, e.special(e.propertyName)], color: x[2] }
  ]
}), ht = {
  background: "#282b2e",
  ...
  settings: {
    ...
    selectionMatch: l.selection,
    gutterBackground: l.background,
    gutterForeground: l.foreground,
    gutterBorder: "transparent",
    lineHighlight: l.activeLine
  },
  styles: [
    { tag: e.keyword, color: l.keyword },
    { tag: [e.name, e.deleted, e.character, e.macroName], color: l.variable },
    { tag: [e.propertyName], color: l.function },
    { tag: [e.processingInstruction, e.string, e.inserted, e.special(e.string)], color: l.string },
    { tag: [e.function(e.variableName), e.labelName], color: l.function },
    { tag: [e.color, e.constant(e.name), e.standard(e.name)], color: l.constant },
    { tag: [e.definition(e.name), e.separator], color: l.variable },
    { tag: [e.className], color: l.class },
    { tag: [e.number, e.changed, e.annotation, e.modifier, e.self, e.namespace], color: l.number },
    { tag: [e.typeName], color: l.type, fontStyle: l.type },
    { tag: [e.operator, e.operatorKeyword], color: l.keyword },
    { tag: [e.url, e.escape, e.regexp, e.link], color: l.regexp },
    { tag: [e.meta, e.comment], color: l.comment },
    { tag: e.tagName, color: l.tag },
    { tag: e.strong, fontWeight: "bold" },
    { tag: e.emphasis, fontStyle: "italic" },
    { tag: e.link, textDecoration: "underline" },
    { tag: e.heading, fontWeight: "bold", color: l.heading },
    { tag: [e.atom, e.bool, e.special(e.variableName)], color: l.variable },
    { tag: e.invalid, color: l.invalid },
    { tag: e.strikethrough, textDecoration: "line-through" }
  ]
}), $t = {
  light: !0,
  ...
  settings: {
    ...
    selectionMatch: m.selectionMatch,
    gutterBackground: m.background,
    gutterForeground: m.foreground,
    gutterBorder: "transparent",
    lineHighlight: m.activeLine
  },
  styles: [
    { tag: e.keyword, color: m.keyword },
    { tag: [e.name, e.deleted, e.character, e.macroName], color: m.variable },
    { tag: [e.propertyName], color: m.function },
    { tag: [e.processingInstruction, e.string, e.inserted, e.special(e.string)], color: m.string },
    { tag: [e.function(e.variableName), e.labelName], color: m.function },
    { tag: [e.color, e.constant(e.name), e.standard(e.name)], color: m.constant },
    { tag: [e.definition(e.name), e.separator], color: m.variable },
    { tag: [e.className], color: m.class },
    { tag: [e.number, e.changed, e.annotation, e.modifier, e.self, e.namespace], color: m.number },
    { tag: [e.typeName], color: m.type, fontStyle: m.type },
    { tag: [e.operator, e.operatorKeyword], color: m.keyword },
    { tag: [e.url, e.escape, e.regexp, e.link], color: m.regexp },
    { tag: [e.meta, e.comment], color: m.comment },
    { tag: e.tagName, color: m.tag },
    { tag: e.strong, fontWeight: "bold" },
    { tag: e.emphasis, fontStyle: "italic" },
    { tag: e.link, textDecoration: "underline" },
    { tag: e.heading, fontWeight: "bold", color: m.heading },
    { tag: [e.atom, e.bool, e.special(e.variableName)], color: m.variable },
    { tag: e.invalid, color: m.invalid },
    { tag: e.strikethrough, textDecoration: "line-through" }
  ]
}), Kt = {
  background: "#303841",
...
const fa = (t) => le[t] || le.strudelTheme;
function ba(t) {
...
let me, H, E, ce = "strudel-theme-vars";
function ya(t) {
  document.getElementById(ce) || (E = document.createElement("style"), E.id = ce, document.head.append(E)), _e(t);
}
function _e(t) {
  if (me === t)
    return;
  me = t, W[t] || console.warn("theme", t, "has no settings.. defaulting to strudelTheme settings");
  const n = W[t] || W.strudelTheme;
  E.innerHTML = `:root {
      ${Object.entries(n).map(([a, o]) => `--${a}: ${o} !important;`).join(`
`)}
    }`, un(n), n.light ? document.documentElement.classList.remove("dark") : document.documentElement.classList.add("dark"), H?.(), H = void 0, n.customStyle && (H = ba(n.customStyle));
}
let T = {};
const xa = (t) => `slider_${t}`;
class ka extends fe {
  constructor(n, a, o, s, i, p, c) {
    super(), this.value = n, this.min = a, this.max = o, this.from = s, this.originalFrom = s, this.to = i, this.step = p, this.view = c;
  }
  eq() {
    return !1;
  }
  toDOM() {
    ...
    return a.type = "range", a.min = this.min, a.max = this.max, a.step = this.step ?? (this.max - this.min) / 1e3, a.originalValue = this.value, a.value = a.originalValue, a.from = this.from, a.originalFrom = this.originalFrom, a.to = this.to, a.style = "width:64px;margin-right:4px;transform:translateY(4px)", this.slider = a, a.addEventListener("input", (o) => {
      ...
      let i = s;
      const p = a.from + a.originalValue.length;
      let c = { from: a.from, to: p, insert: i };
      a.originalValue = i, a.value = i, this.view.dispatch({ changes: c });
      const w = xa(a.originalFrom);
      window.postMessage({ type: "cm-slider", value: Number(s), id: w });
    }), n;
  }
  ignoreEvent(n) {
    return !0;
...
const Re = M.define(), va = (t, n) => {
  t.dispatch({ effects: Re.of(n) });
};
function wa(t, n) {
  return t.filter((a) => a.type === "slider").map(({ from: a, to: o, value: s, min: i, max: p, step: c }) => v.widget({
    widget: new ka(s, i, p, a, o, c, n),
    side: 0
  }).range(
    a
    /* , to */
...
const ja = ge.fromClass(
  class {
    decorations;
    //: DecorationSet
    constructor(t) {
      this.decorations = v.set([]);
    }
    update(t) {
      t.transactions.forEach((n) => {
        ...
        for (let a of n.effects)
          a.is(Re) && (this.decorations = v.set(wa(a.value, t.view)));
      });
    }
  },
  {
...
let Za = (t) => (console.warn("slider will only work when the transpiler is used... passing value as is"), ln(t)), Xa = (t, n, a, o) => (T[t] = n, mn(() => T[t]));
typeof window < "u" && window.addEventListener("message", (t) => {
  t.data.type === "cm-slider" && (T[t.data.id] !== void 0 ? T[t.data.id] = t.data.value : console.warn(`slider with id "${t.data.id}" is not registered. Only ${Object.keys(T)}`));
});
const Le = M.define({
  map: ({ from: t, to: n }, a) => ({ from: a.mapPos(t), to: a.mapPos(n) })
}), Pa = (t, n) => {
  t.dispatch({ effects: Le.of(n) });
};
function Ua(t) {
  return t.sort((n, a) => n.to - a.to).map((n) => v.widget({
    widget: new Ca(n),
    side: 0,
    block: !0
  }).range(n.to));
...
const Fa = [Aa];
function F(t, n) {
  xn(t), n && (cn.prototype[t] = function(a, o = { fold: 1 }) {
    return n(a, o, this);
  });
}
function B(t, n = {}) {
  ...
  return i.width = a * s, i.height = o * s, i.style.width = a + "px", i.style.height = o + "px", Da(t, i), i;
}
F("_pianoroll", (t, n = {}, a) => {
  const o = B(t, n).getContext("2d");
...
const D = {
  isLineWrappingEnabled: (t) => t ? P.lineWrapping : [],
  isBracketMatchingEnabled: (t) => t ? Xe({ brackets: "()[]{}<>" }) : [],
  isBracketClosingEnabled: (t) => t ? Ge() : [],
  isLineNumbersDisplayed: (t) => t ? on() : [],
  theme: fa,
  isAutoCompletionEnabled: qn,
  isTooltipEnabled: In,
  isPatternHighlightingEnabled: Wn,
  isActiveLineHighlighted: (t) => t ? [sn(), rn()] : [],
  isFlashEnabled: Rn,
  keybindings: $n,
  isTabIndentationEnabled: (t) => t ? C.of([$e]) : [],
  isMultiCursorEnabled: (t) => t ? [
    he.allowMultipleSelections.of(!0),
    P.clickAddsSelectionRange.of((n) => n.metaKey || n.ctrlKey)
  ] : []
}, Q = Object.fromEntries(Object.keys(D).map((t) => [t, new Qe()])), Sa = {
  ...
  fontSize: 18
}, _ = kn("codemirror-settings", Sa, {
  encode: JSON.stringify,
  decode: JSON.parse
});
function Na({ initialCode: t = "", onChange: n, onEvaluate: a, onStop: o, root: s, mondo: i }) {
  const p = _.get(), c = Object.keys(Q).map(
    (g) => Q[g].of(D[g](ze(p[g])))
  );
  ya(p.theme);
  let w = he.create({
    ...
    extensions: [
      ...
      ne(ue),
      P.updateListener.of((g) => n(g)),
      ye({ cursorBlinkRate: 0 }),
      L.highest(
        C.of([
          {
            key: "Ctrl-Enter",
...
class Ja {
  constructor(n) {
    ...
    this.code = s, this.root = a, this.miniLocations = [], this.widgets = [], this.drawTime = c, this.drawContext = p, this.onDraw = i || this.draw, this.id = o || qa(), this.solo = S, this.drawer = new hn((u, N, se, O) => {
      const q = u.filter((We) => We.isActive(N));
      this.highlight(q, N), this.onDraw(u, N, O);
    }, c), this.prebaked = g(), w && this.drawFirstFrame(), this.repl = dn({
      ...A,
      id: o,
      onToggle: (u) => {
        A?.onToggle?.(u), u ? (this.drawer.start(this.repl.scheduler), this.solo && document.dispatchEvent(
          new CustomEvent("start-repl", {
            detail: this.id
          })
        )) : (this.drawer.stop(), Y(this.editor, []), re(!0, o));
      },
      beforeEval: async () => {
        re(!0, o), await this.prebaked, await A?.beforeEval?.();
      },
      afterEval: (u) => {
        this.miniLocations = u.meta?.miniLocations, this.widgets = u.meta?.widgets;
        const N = this.widgets.filter((q) => q.type === "slider");
        va(this.editor, N);
        const se = this.widgets.filter((q) => q.type !== "slider");
        Pa(this.editor, se), Y(this.editor, this.miniLocations), A?.afterEval?.(u);
        const O = u.pattern.getPainters().length ? this.drawTime : [0, 0];
        this.drawer.setDrawTime(O), this.drawer.invalidate(this.repl.scheduler);
      }
    }), this.editor = Na({
      root: a,
      initialCode: s,
      onChange: (u) => {
        u.docChanged && (this.code = u.state.doc.toString(), this.repl.setCode?.(this.code));
      },
      onEvaluate: () => this.evaluate(),
      onStop: () => this.stop(),
      mondo: A.mondo
    });
    const ae = this.root.querySelector(".cm-editor");
    ae && (this.root.style.display = "block", U && (this.root.style.backgroundColor = "var(--background)"), ae.style.backgroundColor = "transparent");
    const oe = _.get();
    this.setFontSize(oe.fontSize), this.setFontFamily(oe.fontFamily), this.onStartRepl = (u) => {
      this.solo && u.detail !== this.id && this.stop();
  ...
  setCode(n) {
    const a = { from: 0, to: this.editor.state.doc.length, insert: n };
...
export {
  ...
  _e as activateTheme,
  Le as addWidget,
  _ as codemirrorSettings,
  Sa as defaultSettings,
  _n as flash,
  En as flashField,
  zn as highlightExtension,
  Ln as highlightMiniLocations,
  Na as initEditor,
  ya as initTheme,
  ba as injectStyle,
  Rn as isFlashEnabled,
  Wn as isPatternHighlightingEnabled,
  Qa as markcss,
  F as registerWidget,
  $ as setFlash,
  ke as setMiniLocations,
  Re as setSliderWidgets,
  Da as setWidget,
  W as settings,
  ve as showMiniLocations,
  Za as slider,
  ja as sliderPlugin,
  T as sliderValues,
  Xa as sliderWithID,
  ...
  Ka as themeColors,
  le as themes,
  to as toggleBlockComment,
  ao as toggleBlockCommentByLine,
  oo as toggleComment,
  so as toggleLineComment,
  Y as updateMiniLocations,
  va as updateSliderWidgets,
  Pa as updateWidgets,
  Fa as widgetPlugin
};
